...
function añadircolumna() {
    ...
    for (let i = 0; i < filas.length; i++) {//recorro las filas
        ...
        celda.innerHTML = "Fila" + (i + 1) + " Celda " + (celdas.length + 1);//pongo el numero de celda y fila
 ...
 */
 //solucion
    ...
    boton3.type="button";
    boton3.id="addContenido";
    boton3.value="Añadir contenido";
    addcolumna.insertAdjacentElement("afterend",boton3);
    addContenido.addEventListener("click",añadirContenido);
    
    function añadirContenido(){
    try{
        let nfila=prompt("Indica el número de fila(1-"+tabla.rows.length+"): ","1");
        nfila=parseInt(nfila)-1;
        console.log(nfila);
       let  ncol=prompt("Indica el número de columnas(1-"+tabla.rows[nfila].cells.length+"): ","1");
       ncol=parseInt(ncol)-1;
       console.log(ncol);
       /**
        * DIFERENCIAS ENTRE INNERHTML Y CREATE TEXTNODE
        * INNERHTML:SUSTITUYE TODO EL CONTENIDO
        * TEXTNODE:SE AÑADE CON APPENCHILD Y AL FINAL DE LO QUE SE HAYA ESCRITO
        */
       let contenido=prompt("¿Que quieres poner golfo :) ?");
       tabla.rows[nfila].cells[ncol].innerHTML = contenido;
